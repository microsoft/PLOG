...
def main():
    ...
    def preprocess_logic2text_function(examples):
        ...
        tables = list(map(_construct_table, zip(examples['table_header'], examples['table_cont'])))
    ...
    def prepare_single_data(args, output_dir, split, test_name):
        with open(os.path.join(args.data_dir, f'{split}.json'), 'r') as fin, \
            ...
            for i, (d, s) in enumerate(zip(data, sents)):
                d_new = {}
                d_new['id'] = i + 1
                d_new['table_header'] = d['table_header']
                d_new['table_cont'] = d['table_cont']
                d_new['sent'] = s.rstrip()
                d_new['label'] = 1
